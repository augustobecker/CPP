...
void	Bureaucrat::signForm( AForm &document ) const
{
    ...
    {
        std::cout << "EXCEPTION: " << this->_name << " couldn’t sign " << document.getName() << " because Bureaucrat's grade isn't enough to sign this Form" << std::endl;
        return;
    }
...
void Bureaucrat::executeForm( AForm const & document ) const
{
    ...
    {
        std::cout << "EXCEPTION: " << this->_name << " couldn’t sign " << document.getName() << " because " << e.what() << std::endl;
        return;
    }
